...
function transformToLis(obj){
  // Solution code here...
  return Object.entries(obj).map(keypair => `<li>${keypair.join(': ')}</li>`);
}
...
const count = (target, input) => {
  // Solution code here...
  let matchArr = input.flat().filter(num => num === target);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(nestedArr => {
    return nestedArr.filter(num => num % 5 === 0 && typeof number === 'number').map(filteredNumber => Math.pow(2, filteredNumber));
  });
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortestSoFar, nextChar) => +shortestSoFar.height < +nextChar.height ? shortestSoFar : nextChar).name;
};
