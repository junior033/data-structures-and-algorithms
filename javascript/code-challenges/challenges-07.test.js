...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here... ** received help from TA Justin **
  recipe.ingredients.forEach(element => {
    let idxOne = element.indexOf(' ') + 1;
    let ingredient = element.slice(idxOne);
    let idxTwo = ingredient.indexOf(' ') + 1;
    let newIngredient = ingredient.slice(idxTwo);
